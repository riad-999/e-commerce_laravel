    ...
    <div class="desk:grid grid-cols-6 gap-8 bg-gray-100">
        ...
        <main class="col-span-5 pt-8 px-4">
            <h4 class="font-body font-semibold mb-8">categories</h4>      
                ...
                autocomplete="off" action="{{ route('update-category',$category->id) }}">
                    @csrf
                    @method('PATCH')
                    <h5 class="font-body font-semibold mb-8">modifier {{$category->name}}</h6>
                    <x-form.input name="name" value="{{ old('name') ? old('name') : $category->name}}" label="nom" 
                    class="w-full" :edit="true" :disabled="true" />
                    <x-form.textarea name="description" :content="old('description') ? old('description') : $category->description" 
                    label="description" class="w-full" :edit="true" :disabled="true" />
                    <div>
                        <a href="{{route('categories')}}" class="btn">liste</a>
